The `.data` section of a PE file contains the initialized global and local variables.

This section is readable and writeable, which makes it suitable for encrypted payload that requires decryption during runtime.

If the payload is global or local variable it will be stored in the .data section.

The demonstration of this with below code.

```c
	#include <Windows.h>
	#include <stdio.h>

	unsigned char Data_Section[] = {
		
	};

	int main() {
		printf("[i] Data_RawData var : 0x%p \n", Data_Section);
		printf("[#] Press <Enter> to Quit ....");
		getchar();
		return 0;
	}
```

Now using PE-bear & 64dbg to view the .data section and make sure our payload is actually there.
<img width="1373" height="449" alt="image" src="https://github.com/user-attachments/assets/953a1d33-379e-4e5e-85e5-c8124dacc68f" />

The image above shows the output from the code snippet in 64dbg. we can see in memory map that address is getting marked to `data` and it has `RW—` which means Read and Write permission.

<img width="629" height="265" alt="image" src="https://github.com/user-attachments/assets/21e33ef6-a5df-493f-a622-daecc0474bae" />

The image above shows the output from the PE-bear this shows the payload being stored in .data.
